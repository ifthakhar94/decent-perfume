...
- **Source of truth**: `docs/features/design-system-glassmorphism.md` and `src/components/providers/AntdThemeProvider.tsx`. Reference `src/components/storefront/store-home-hero.tsx`, `storefront-shell.tsx` (header + footer), and `site-header` for storefront composition; `ComingSoonPage` is dashboard-only.
