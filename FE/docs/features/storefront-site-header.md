...
Global chrome for the **customer-facing** app only. Implemented as `SiteHeader` inside `StorefrontShell`, which wraps the `(store)` route group. **`/dashboard` does not use this header** (merchant area is separate).
...
| Piece                           | Role                                                          |
| ------------------------------- | ------------------------------------------------------------- |
| `storefront-shell.tsx`          | Scene background + `SiteHeader` + `<main id="main-content">`. |
